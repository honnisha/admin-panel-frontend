...
export default {
  ...
  created() {
    ...
    if (this.sectionData.meta.inline_type) {
      console.error(`Page ${this.viewname} tried to render as List, but its inline.`)
      this.$router.push({ path: '/404' })
      return
    }
  ...
  methods: {
    ...
    getHeaders() {
      ...
      const serializer =  this.sectionData.meta.serializer
      if (!serializer) {
        return result
      }

      const settings = getViewsetsSettings(this.viewname, serializer)

      if (serializer && !this.sectionData.meta.fields_list) {
        for (const [name, field] of Object.entries(serializer)) {
      ...
      for (const name of this.sectionData.meta.fields_list) {

        const field = serializer[name]
        if (!field) {
    ...
    canDisplayInList(field, field_name) {
      if (!field.write_only && (!this.sectionData.meta.fields_list || this.sectionData.meta.fields_list.indexOf(field_name) !== -1)) {
        return true
      }
      return false
    },
